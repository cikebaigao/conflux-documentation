...
title: My First Transaction -> What is Transaction
keywords:
  - transaction
---
...
cfx_sendRawTransaction RPC link
why transaction is pending
...
from 字段标识了交易的发送者，且交易需要在发送前由使用发送者的私钥进行签名，以确保他人无法冒充交易发送者。`from`字段可以填写任何你控制私钥的账户地址。不过还需要注意的是，账户中需要有足够的余额用于支付交易开销（包括转账金额与交易费），否则交易会被RPC拒绝造成发送失败。需要说明的是，特殊情况下，Conflux的代付机制能够让其他账户代付交易费，从而允许余额为0的账户发送交易。

> 事实上，交易的from字段最终并不会直接包含在编码后的交易中，一般而言，SDK等工具编码交易时会将from字段从交易中去除，并根据from字段使用对应的私钥签名交易。交易的验证者之后可以从签名中恢复交易的发送者。
